...
private lemma List.exists_map_char_of_not_mem_eos
...
omit [FinEnum Γ] [DecidableEq Γ] in
private lemma List.exists_map_char_append_eos_of_mem_eos
...
omit [FinEnum Γ] [FinEnum α] [DecidableEq β] [DecidableEq Γ] in
private lemma BuildDetokLexer_whitespace_singleProducible_start
    [Vocabulary α β]
    {tnonwhite twhite : α} {qnonwhite qwhite : σa}
    (spec : LexerSpec α Γ σa)
...
omit [DecidableEq Γ] in
private lemma PDA.exists_accepts_extension_of_pruned_evalFull_nonempty
    (P : PDA Γ π σp) (hpruned : P.pruned)
...
theorem GCDChecker_checkerAllows_imp_checkerAccepts_extension
  ...
  | inr hsnoc =>
      ...
      · simp [acceptedWord, List.concat_eq_append]
