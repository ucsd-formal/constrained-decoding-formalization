...
/-- A vocabulary of tokens `V` over characters `Input`.
...
-/
class Vocabulary (Input : Type u) (V : Type v) [BEq Input] where
  flatten : V → List Input
  embed : Input → V
...
and checker constructions over `Ch Input`.
-/
instance {Input V} [DecidableEq Input] [BEq Input] [b : Vocabulary Input V] : Vocabulary (Ch Input) (Ch V) where
